...
The paper (gravitas) describes how we can compute all possible combinations of cyclic time granularities. If we have $n$ periodic linear granularities in the hierarchy table, then $n(n-1)/2$ circular or quasi-circular cyclic granularities could be constructed. Let $N_C$ be the total number of contextual circular, quasi-circular and aperiodic cyclic granularities that can originate from the underlying periodic and aperiodic linear granularities. Assuming a numeric response variable, the graphics display distributions compared across combinations of these cyclic granularities, one placed at x-axis and the other on the facet. With this design, potentially, there are $^{N_C}P_2$ possible pairwise plots, where each plot would display a pair of cyclic granularities. Now, this is large and overwhelming for human consumption.
...
large, and scatterplot matrices (SPLOMs) could easily run out of pixels when presenting high-dimensional data. [@ScagExplorer], [@otherref] provides a solution to this, where few characterizations help us to locate anomalies for further analysis or search for
similar distributions in a "large" SPLOM with more than a hundred dimensions.


<!-- harmonies and why it is not enough -->
The paper (gravitas) narrows down our search to harmonies and removes clashes without losing any data perspectives. However, even after excluding clashes, the list of harmonies left could be extensive and overwhelming for human consumption. Hence, there is a need to rank the harmonies based on how well they capture the variation in the measured variable and reduce the number of harmonies to include only those for which variation within different categories is significant.


<!-- in the same group would be important to bring out different patterns of the data. -->

Here, we have two main objectives:

- To choose harmonies for which distributions of categories are significantly different 
- To rank the selected harmonies from highest to lowest variation in the distribution of their categories. The idea here is to rate a harmony pair higher if this variation between different x-axis variable levels is higher on an average across all levels of facet variables.
