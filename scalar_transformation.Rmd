...
N01 %>% 
  ...
  ylab("wpd_norm1 = (wpd - intercept)/log(nx*nfacet)")
...
# Reduce the heteroskedasticity by the transformation y - a - b*log(n)


In Figure \ref{fig:new-transform}, we see that the transformed wpd value $wpd_{norm2} = wpd - intercept - slope*log(nx*nfacet)$ has lower heteroscedasticity as compared to $wpd_{norm1}$ when plotted across $log(nx*nfacet)$.


```{r new-transform, echo = TRUE, fig.cap = "$wpd_{norm2}$ is plotted against log(nx*nfacet). The relationship is horizontal with reduced heteroscedasticity."}
N01 %>% 
...
```{r dist-new-transform, echo = TRUE, fig.cap = " The distribution of $wpd_{norm2}$ is plotted. The distributions are more similar across different nx and nfacets as compared to $wpd_{norm1}$."}
N01 %>% 
