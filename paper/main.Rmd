...
Consider two cyclic granularities $A$ and $B$, such that $A = \{ a_j: j = 1, 2, \dots, J\}$ and $B = \{ b_k: k = 1, 2, \dots, K\}$ with $A$ placed across x-axis and $B$ across facets. Let the pairwise distances between combinations be denoted as $d_{(j,k), (j', k')} = JSD(a_{j}b_{k}, a_{j'}b_{k'})$. If $b_{k} = b_{k'}$, distances considered are within-facet. Otherwise they are between-facet.

```{r distance-explain, fig.cap = "Mapping to data space to graphic space shown. Cyclic granularity A is mapped to x-axis and B is mapped to facets and the dotted lines represent the distances between different categories. Panel 1) and 2) show the between-facet distances. Panel 3) and 4) are used to illustrate within-facet distances when categories are un-ordered or ordered respectively. When categories are ordered, distances should only be considered for consecutive x-axis categories. Between-facet distances are distances between different facet levels for the same x-axis category, for example, distances between {($a_1$,$b_1$) and ($a_1$, $b_2$)} or {($a_1$,$b_1$) and ($a_1$, $b_3$)}."}
...
d*_{(j,k), (j', k')} =
\begin{cases}
\lambda * d_{(j,k), (j', k')},& \text{if } b_{k} = b_{k'}\\
(1-\lambda) * d_{(j,k), (j', k')},              & \text{otherwise}
\end{cases}
\end{equation}
