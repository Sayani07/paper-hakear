...
Observations are generated from a N(0,1) distribution for each combination of $nx$ and $nfacet$ from the following sets: $nx = nfacet = \{2, 3, 5, 7, 14, 20, 31, 50\}$ to cover a wide range of levels from very low to moderately high. That is, data is being generated for each of the panels $\{nx = 2, nfacet = 2\}, \{nx = 2, nfacet = 3\}, \{nx = 2, nfacet = 5\},  \dots, \{nx = 50, nfacet = 31\}, \{nx = 50, nfacet = 50\}$. For each of the $64$ panels, $ntimes = 500$ observations are drawn for each combination of the categories. That is, if we consider the panel  $\{nx = 2, nfacet = 2\}$, $500$ observations are generated for each of the combination of categories from the panel, namely, $\{(1, 1), (1, 2), (2, 1), (2, 2)\}$. The value of $\lambda$ is set to $0.67$. $nsim = 200$, which means each of these scenarios are run $200$ times by permuting the data in each of the panels. And hence $200$ values of $wpd_{raw}$ are obtained for each of those 64 panels. 


### Permutation approach

The permutation approach ensures that the distribution of the normalised weighted pairwise distance measure has the same mean and standard deviation across all combinations of $nx_i$ and $nfacet_j$. Thus, the normalised distances is computed as follows: $x^{norm}_{k} =  (x_{k} - mean_l(x_{k,l}))/sd_l(x_{k, l})$,  $x^{norm}_{k}$ is the value of the $wpd_{norm}$ for the $k^{th}$ panel.

While this works successfully to make the mean and standard deviation across different nx and nfacet (as seen in Figure \ref{fig:}), it is computationally heavy and time consuming, and hence less user friendly when being actually used in practice. Hence, we propose another approach to normalisation which is more approximate than exact but still has the same accuracy when compared to the permutation approach.
...
Complete randomness in the measured variable indicates that the process follows a homogeneous underlying distribution over the whole time series, irrespective of the different cyclic granularities; which essentially implies there is no interesting distinct across any particular time deconstruction. We can remove the harmonies for which no interesting patterns are observed through a randomisation permutation method. Essentially, this method is
based on the generation of randomly chosen reassignments
(permutations) of the data across different cyclic granularities and the the computation for each permutated data of the theoretical distribution of $wpd_{norm}$ expected under the null hypothesis. The percentages of the theoretical distribution greater than or equal to the respective observed $wpd_{norm}$ values are calculated and used to obtain the P value. The procedure for the permutation test is:


**Assumption:** random permutation without considering ordering 
(global)

1. Given the data; $\{v_t: t=0, 1, 2, \dots, T-1\}$, the $wpd_norm$ is computed and is represented by $wpd_{obs}$.

2. From the original sequence a random permutation is obtained: $\{v_t^*: t=0, 1, 2, \dots, T-1\}$.

3. $wpd_norm$ is computed for all random permutation of the data and is represented by $wpd_{sample}$.

4.  Steps (2) and (3) are repeated a large number
of times M (e.g. 200).

5. For each permutation, one $wpd_{sample}$ value is obtained.
...
### Advantage over taditional feature based methods
