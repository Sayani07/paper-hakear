...
Let $y_{i, t}$ denote the electricity demand for $i^{th}$ household for time period $t$. This is expected to have an assymmetrical distribution as could be seen in \ref{fig:raw-nqt} and the Normal score transform has been applied to make it more treatable. Let $y*_{i, t}$ denote the normal-quantile transformed electricity demand for $i^{th}$ household for time period $t$.

**Characterizing empirical distributions through quantiles** ( write in the methodology maybe)

Let $A = \{ a_j: j = 1, 2, \dots, J\}$ be a cyclic granularity of interest. Let 
$q^{i, p}_{A, j}$ is the quantile with probability $p$ for the $j^{th}$ category of the cyclic granularity $A$ of the $i^{th}$ household.
Sample quantiles were computed at $p = 0.01, 0.02, \dots, 0.99$ for each household $i \in i = 1, 2, 3, 4$  for each category $j \in j = 1, 2, \dots, J$ and all $A \in N_C$.
...
all_harmony2 <- hakear::rank_harmonies(id2_tsibble,
...
library(tictoc)
tic()
all_harmony4 <- hakear::rank_harmonies(id4_tsibble,
...
## Australian smart meter data set
