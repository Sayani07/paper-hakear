...
## Distribution of wpd under $D_{null}$
...
The Normal Score Transform or NQT has been applied in various fields of geo-statistics in order to make the mostly asymmetrical distributed real world measured variables more
...
**Objective:** 1. Study the behavior of wpd under different initial distributions for different $nx$ and $nfacet$.
...
## Choice of tuning parameters and effect of different increments
...
```{r tuning-one-omega, fig.caption = "WPD from two designs are plotted for different values of lambda for different x and facet categories. For omega = 1, the designs intersect for 0.6<lamba<=0.75, whereas for higher omega, design intersects at lambda = 0.5", eval = FALSE}
...
```{r tuning-parameter-intersection, eval = FALSE}
