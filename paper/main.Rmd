...
Figure \ref{fig:raw} shows that both the location and scale of the distributions change across panels. This is not desirable under $D_{null}$ as it would mean comparisons of $wpd$ values is not appropriate across different $nx$ and $nfacet$. \ref{tab:glm-tab} gives the summary of the generalized linear model to capture the relationship between $wpd_{raw}$ and number of comparisons. Thus, the model considered is $wpd_{l,s} = 23.69448+-1.02357*log(nx * nfacet)) + e$. The intercepts are similar independent of the starting distribution (Please refer to supplementary).
