...
wpd_{norm} =
\begin{cases}
wpd_{norm}^{perm},& \text{if } J, K <=5\\
wpd_{norm}^{glm-scaled},              & \text{otherwise}\\
\end{cases}
\end{equation}
...
elec$date_time <- as.Date(elec$date_time)

elec_linear <- elec %>% 
  ggplot() +
  geom_line(aes(x = date_time, y = kwh, group = time)) +
...
## Smart meter application to 8 datasets from energy competition
