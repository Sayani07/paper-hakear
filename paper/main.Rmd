...
```{r sim_all, echo = FALSE, eval = FALSE}
...
```{r rank-harmonies, eval = FALSE}
all_harmony2 %>%
  mutate(rank = row_number()) %>% 
  left_join(all_harmony4 %>%  mutate(rank = row_number()), by = c("facet_variable", "x_variable")) %>% 
  select(facet_variable, x_variable,
          rank.x, rank.y, wpd.x, wpd.y) %>% 
  mutate(rank.x = if_else(rank.x<=3, paste(rank.x, "***",sep = ""),as.character(rank.x))) %>% 
   mutate(rank.y = if_else(rank.y<=3, paste(rank.y, "***",sep = ""),as.character(rank.y))) %>% 
  mutate(wpd.x = round(wpd.x, 3),
         wpd.y = round(wpd.y, 3)) %>% 
  rename("rank:id2" = "rank.x",
         "rank:id4" = "rank.y",
         "wpd:id2" = "wpd.x",
         "wpd:id4" = "wpd.y"
         )  %>% 
...
library(tictoc)
tic()
all_harmony4 <- hakear::select_harmonies(id4_tsibble,
