...
Most of the behavior was studied via simulation. The simulations explore how wpd performs under various simulations and its limitations. Numerous simulations were carried out in a wide range of scenarios. Same scenarios were used, when possible with normalized wpd, keeping all other simulation variables constant.

The factors that are thought to potentially have some impact on wpd are as follows: 
...
**Objective:**  Study the behavior of wpd under different initial distributions for different $nx$ and $nfacet$

In this section, the effect of all the factors are evaluated under the null design $D_{null}$, that is, there is no difference in distribution between any facet or x-category. It is further assumed that the values of the following factors are fixed to $nsim = 200$, $n = 500$, $\lambda = 0.67$ and $\omega = 0$, since there is no significant differences between distributions in the null case.
...
The Normal Score Transform or NQT has been applied in various fields of geo-statistics in order to make most asymmetrical distributed real world measured variables more
...
wpd is computed with the initial distribution set as N(0,1), N(5, 1), N(0,5), G(0.5, 1) and G(2, 1).
...
Figure \ref{fig:normal-3diff} shows distribution of wpd for N(0,1), N(5,1) and N(0,5) distribution to see the effect of changing location and scale on the distribution of wpd. It seems like the  distribution changes across different facet and x levels but look same for different  normal distributions, which implies wpd value is unaffected by the change in location and scale of the the normal distribution. 

Figure \ref{fig:mean-sd-ns-nfacet} shows how mean and sd of the distribution of wpd changes with the increasing x and facet levels. It seems like both mean and standard deviation are affected more by change in the x-axis levels than the facet levels.

```{r normal-diff, fig.cap="Ridge plots of raw wpd is shown for N(0,1) and N(5,1) distribution. The densities change across different facet and x levels but look same for the two distributions, which implies wpd value is unaffected by the change is mean value of the normal distribution", eval = FALSE}
knitr::include_graphics(here::here("simulations/null_design/figs/normal_ridge_nxbynfacet_wpd_N01.png"))
```

```{r normal-3diff, fig.cap="Ridge plots of raw wpd is shown for N(0,1), N(5,1) and N(0,5) distribution. The densities change across different facet and x levels but look same for the two distributions, which implies wpd value is unaffected by the change is mean value of the normal distribution", eval = FALSE}
...
Objective: Study the effect of different tuning parameters $\lambda$ and $\omega$     with the help of two alternate designs ($vary_x$ and $vary_f$) for different $nx$ and $nfacet$. The $\lambda$ for which the two designs intersect will then be chosen as the optimal $\lambda$ that could then be weighed appropriately to up-weigh the within-facet distances and down-weigh the between-facet distances. Figure \ref{fig:tuning-one-omega} are plotted for different values of $\lambda$ for a relatively small and higher $\omega$. Designs mostly intersect at $\lambda = 0.5$ for the higher value of $\lambda$. Figure \ref{fig:tuning-parameter-intersection} shows how the value of lambda changes with increasing omega.
...
```{r tuning-one-omega, fig.caption = "WPD from two designs are plotted for different values of lambda for different x and facet categories. For omega = 1, the designs intersect for 0.6<lamba<=0.75, whereas for higher omega, design intersects at lambda = 0.5"}
knitr::include_graphics(here::here("simulations/tuning_param/figs/fixed_omega.png"))
```

```{r tuning-parameter-intersection, fig.cap="For most panels it is observed that the most common values for which the "}
...
```{r, eval = FALSE}
knitr::include_graphics(here::here("simulations/null_design/figs/normalised","gamma_ridge_nxbynfacet.png"))
```
...
```{r}
knitr::include_graphics(here::here("paper/Figs/simulation.png"))
```


Observations from Normal(5, 10) and Gamma(0.5, 0.2) are3 chosen in all the designs.

<!-- . the null distribution are chosen as:   -->
...
```{r norm-data, fig.cap = "Distribution of $MMPD_{norm}$ across different levels of facet and x-axis. These distributions should look the same if normalisation worked." }
mmpd_norm = read_rds("../hakear-drake/data/data_norm_low_new.rds") 
hist_plot = mmpd_norm %>% 
      ggplot() +
...
### Performance of normalised MMPD


```{r}
data1  = sim_panel(nx = 2, nfacet = 3, ntimes = 500)
simulated_data <- sim_panel(nx = 2,
nfacet = 3,
ntimes = 500,
sim_dist = rep(dist_normal(seq(5, 15, 5), 5), each = 2))
data2 = simulated_data
data4 = sim_panel(nx = 2,
nfacet = 3,
ntimes = 500,
sim_dist = dist_normal(seq(5,30, 5), 5))
data3 = data2 %>% rename("temp" = "id_facet") %>% rename("id_facet" = "id_x") %>% rename("id_x" = "temp")
...
set.seed(1000)
r1 <- compute_mmpd_norm(data1, "id_x", "id_facet", response = sim_data, nperm = 200)

r2 <- compute_mmpd_norm(data2, "id_x", "id_facet", response = sim_data, nperm = 200)

r3 <- compute_mmpd_norm(data3, "id_x", "id_facet", response = sim_data, nperm = 200)

r4 <- compute_mmpd_norm(data4, "id_x", "id_facet", response = sim_data, nperm = 200)

r1prime <- compute_norm_pairwise_max(data1, "id_x", "id_facet", response = sim_data, nperm = 200)

r2prime <- compute_norm_pairwise_max(data2, "id_x", "id_facet", response = sim_data, nperm = 200)

r3prime <- compute_norm_pairwise_max(data3, "id_x", "id_facet", response = sim_data, nperm = 200)

r4prime <- compute_norm_pairwise_max(data4, "id_x", "id_facet", response = sim_data, nperm = 200)
...
p1 <- data1 %>%
  ggplot(aes(x = as.factor(id_x), y = sim_data)) + facet_wrap(~id_facet) + geom_boxplot() +xlab("")

p2 <- data2 %>% ggplot(aes(x = as.factor(id_x), y = sim_data)) + facet_wrap(~id_facet) + geom_boxplot() +xlab("")

p3 <- data3 %>% ggplot(aes(x = as.factor(id_x), y = sim_data)) + facet_wrap(~id_facet) + geom_boxplot() +xlab("")

p4 <- data4 %>% 
  ggplot(aes(x = as.factor(id_x), y = sim_data)) + facet_wrap(~id_facet) + geom_boxplot() +xlab("")


gridExtra::grid.arrange(gridExtra::arrangeGrob(p1, top = paste("MMPD", round(r1, 3), "pairwise", round(r1prime, 3))),
                        gridExtra::arrangeGrob(p2, top = paste("MMPD", round(r2, 3), "pairwise", round(r2prime, 3))),
                        gridExtra::arrangeGrob(p3, top = paste("MMPD", round(r3, 3), "pairwise", round(r3prime, 3))),
                        gridExtra::arrangeGrob(p4, top = paste("MMPD", round(r4, 3), "pairwise", round(r4prime, 3))),
                        ncol = 2, nrow=2)
