...
# w could be w1 and w2. For simplicity, kept w

# functions to generate distribution vector across combinations so that they are same across x and different across facet

sim_varf_normal_varyboth = function(nx, nfacet, mean, sd, w )
{
  rep(dist_normal((mean + seq(0, nfacet-1, by  = 1)*w), 
                  (sd + seq(0, nfacet-1, by  = 1)*w)), each = nx)
}

# functions to generate distribution vector across combinations so that they are same across facet and different across x

sim_varx_normal_varyboth = function(nx, nfacet, mean, sd, w)
{
  rep(dist_normal((mean + seq(0, nx-1, by  = 1)*w), 
                  (sd + seq(0, nx-1, by  = 1)*w)), nfacet)
}

# functions to generate distribution vector across combinations so that they are different across both facet and x

sim_varall_normal_varyboth = function(nx, nfacet, mean, sd, w)
{
  dist_normal((mean + seq(0, (nx*nfacet - 1), by  = 1)*w), 
              (sd + seq(0, (nx*nfacet - 1), by  = 1)*w))
}
