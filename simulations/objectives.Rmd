...
## Objectives:
 ...
 (1A) to understand how the measure (raw or normalised) changes across different levels of facets and x-axis for fixed $nsim$, $nperm$ and $\lambda$ in null case. Original values are **not scaled**, sim_table has just nx and nfacet. Description of parameters: 
...
(1B)  Original values are **scaled** and other parameters look exactly like (A)

(2)  Original values are **scaled** and $weights (dist_{within-facet}, dist_{across-facet}) = (\lambda, 1 - \lambda)$, other parameters like (A), for $lambda = seq(0.1, 0.9, 0.05)$
