  ...
  data_w <- mclapply(range_w, function(w){
    data_y <- mclapply(range_mean, function(y){   
      ...
      data <- sim_panel(nx = 3,
                        nfacet = 2,
                        ntimes = 500,
                        sim_dist = sim_varx_normal(nx = 3, nfacet = 2, mean = y, sd = 2, w = w)) %>%
        unnest(c(data))
  ...
  data_w <- mclapply(range_w, function(w){
    data_y <- mclapply(range_mean, function(y){   
      ...
      data <- sim_panel(nx = 3,
                        nfacet = 2,
                        ntimes = 500,
                        sim_dist = sim_varf_normal(nx = 3, nfacet = 2, mean = y, sd = 2, w = w)) %>%
        unnest(c(data))
...
set.seed(9999)
data_varx <- lapply(range_lambda, function(x){
  data_w <- lapply(range_w, function(w){
    data_y <- lapply(range_sd, function(y){   
      # generate the data
      data <- sim_panel(nx = 3,
                        nfacet = 2,
                        ntimes = 500,
                        sim_dist = sim_varx_normal(nx = 3, nfacet = 2, mean = 5, sd = y, w = w)) %>%
        unnest(c(data))
...
set.seed(9999)
data_varf <- lapply(range_lambda, function(x){
  data_w <- lapply(range_w, function(w){
    data_y <- lapply(range_sd, function(y){   
      # generate the data
      data <- sim_panel(nx = 3,
                        nfacet = 2,
                        ntimes = 500,
                        sim_dist = sim_varf_normal(nx = 3, nfacet = 2, mean = 0, sd = y, w = w)) %>%
        unnest(c(data))
